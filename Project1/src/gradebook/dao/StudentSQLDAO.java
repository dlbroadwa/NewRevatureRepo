...
public class StudentSQLDAO implements UserDAO<User,String> {
	...
	private static StudentSQLDAO instance;

	private StudentSQLDAO() {}
	
	public static StudentSQLDAO getInstance() {
		if (instance == null)
			instance = new StudentSQLDAO();
		return instance;
	}
	...
	public User getUser(String user_id) {
		...
		try {
			...
			String sql_query = "select * from gradebook.students where student_id=?";
	...
	public boolean updateUser(User user) {
		...
		try {
			connection = ConnectionProvider.getConnection();
			String sql_query = "update gradebook.students set first_name=?, last_name=?, email=?, password=? where student_id=?";
			ps = connection.prepareStatement(sql_query);
